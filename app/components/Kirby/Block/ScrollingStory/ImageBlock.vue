...
const galleryId = computed(() => {
  const hash =
    props.block.image.url?.split('/').pop() ||
...
<template>
  <figure
    class="my-6 flex-grow w-full"
    :class="[isLightboxEnabled ? 'pswp-gallery' : '', getFigureBorderClasses]"
  ...
  >
    ...
    <component
      ...
      :rel="
        !isLightboxEnabled && block.link ? 'noopener noreferrer' : undefined
      "
