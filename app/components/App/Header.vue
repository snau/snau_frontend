...
<template>
  <header class="fixed top-8 left-0 right-0 z-50 w-full mx-auto">
    <nav
      ...
      <div class="flex items-center gap-6 relative">
        ...
        <div class="relative z-10 flex items-center gap-6 transform-gpu will-change-transform">
          ...
          <!-- Desktop Navigation -->
          <div class="hidden md:block transition-opacity duration-300" :class="{
            'opacity-0': !isHeaderVisible,
            'opacity-100': isHeaderVisible,
          }">
            <div class="px-3">
              <UNavigationMenu variant="pill" orientation="horizontal" :items="menuItems" :ui="{
                list: 'gap-6',
                linkLabel: 'py-0',
      ...
      <UButton aria-controls="mobile-nav" :aria-expanded="isOpen" aria-label="Mobile menu toggle"
        class="absolute right-4 scale-110 md:hidden transition-opacity duration-300" :class="{
          'opacity-0': !isHeaderVisible,
          'opacity-100': isHeaderVisible,
        }" :icon="isOpen ? 'i-tabler-x' : 'i-tabler-menu-2'" size="xl" color="neutral" variant="ghost"
        @click="toggle" />
    </nav>
    ...
    <div id="mobile-nav"
      ...
      <div v-if="isOpen"
        class="absolute inset-0 mt-2 rounded-2xl shadow-md border border-white/25 dark:border-white/15 backdrop-blur-md backdrop-saturate-150 bg-white/90 dark:bg-black/25">
      </div>

      <div class="overflow-hidden min-h-0 relative z-10 transition-all duration-300 ease-[var(--ui-easing)]"
        :class="isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-1'">
        <UNavigationMenu :key="navigationMenuKey" variant="pill" orientation="vertical" :items="menuItems" :ui="{
          root: 'bg-transparent',
